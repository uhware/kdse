...
static void usage(FILE *stream)
{
    (void)fprintf(stream,
        "Usage:\n"
        "  kdse validate VALUE\n"
        "  kdse canonicalize VALUE\n"
        "  kdse profile VALUE\n"
        "  kdse compute VALUE --input N --threshold T --loss-percent P\n"
        "\n"
        "VALUE accepts decimal, 0x hexadecimal, or 0b binary notation.\n"
        "KDSE-8 bit 7 is unassigned and ignored.\n");
}

static bool parse_binary(const char *text, kdse8_t *value)
{
    uint16_t result = 0u;
    size_t digits = 0u;
    ...
    while (*text != '\0') {
        ...
        result = (uint16_t)((result << 1u) | (uint16_t)(*text - '0'));
        ++text;
    }
    if (digits == 0u) {
        return false;
    }
    *value = (kdse8_t)result;
    return true;
}

static bool parse_value(const char *text, kdse8_t *value)
{
    char *end;
    ...
    if (errno != 0 || end == text || *end != '\0' || result > UINT8_MAX) {
        return false;
    }
    *value = (kdse8_t)result;
    return true;
...
static void print_bits(uint8_t payload)
{
    const uint8_t length = kdse8_payload_length(payload);
    uint8_t position;
    ...
    for (position = 0u; position < length; ++position) {
        ...
        (void)putchar(((payload >> shift) & UINT8_C(1)) != 0u ? '1' : '0');
    }
}

static void print_identity(kdse8_t value)
{
    const uint8_t payload = kdse8_payload(value);

    (void)printf("container: %" PRIu8 "\n", value);
    (void)printf("unassigned-bit: %u (ignored)\n",
                 (unsigned)((value >> 7u) & UINT8_C(1)));
    (void)printf("payload-decimal: %" PRIu8 "\n", payload);
    (void)fputs("payload-bits: ", stdout);
    print_bits(payload);
    (void)putchar('\n');
}

static int command_validate(kdse8_t value)
{
    const kdse_status_t status = kdse8_validate(value);
...
static int command_canonicalize(kdse8_t value)
{
    kdse8_t ordered;
    kdse_status_t status = kdse8_canonicalize(value, &ordered);
    ...
    print_bits(kdse8_payload(value));
    (void)fputs("\nordered-payload: ", stdout);
    print_bits(ordered);
    (void)printf("\nordered-decimal: %" PRIu8 "\n", ordered);
    return EXIT_SUCCESS;
}

static int command_profile(kdse8_t value)
{
    kdse8_profile_t profile;
    kdse_status_t status = kdse8_decode_profile(value, &profile);
    uint8_t depth;
...
static int command_compute(kdse8_t value, int argc, char **argv)
{
    double input = 0.0;
    ...
    output = kdse8_compute(value, input, threshold,
                                loss_percent / 100.0);
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3 || !parse_value(argv[2], &value)) {
        (void)fputs("VALUE must be an unsigned byte (0 through 255)\n", stderr);
        return 2;
    }
